...
class Parser:
    ...
    def statement(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected 'return', 'continue', 'break', 'var', 'if', 'for', 'while', 'func', int, float, identifier, "
                "'+', '-', '(', '[' or 'not' "
            ))
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_pop(self, exec_ctx):
        ...
        try:
            ...
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                'Element at index {} could not be popped because index is out of range.'.format(index.value),
                exec_ctx
            ))
